...
- `allow_llm_hangup` *(Optional)*: Expose a SIP-specific `sip__hang_up_current_call` tool to this call's Assist LLM (default: `false`). When called, it schedules the BYE after the current intent turn and its TTS reply finish, allowing other requested tools and a brief goodbye to run first. A 30-second timeout prevents a stalled turn from leaving the call open.
